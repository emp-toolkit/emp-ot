...
// ePrint 2022/1431, Figure 3). Sibling pair at every non-leaf:
//   left  = H(parent)
//   right = parent XOR left
// where H is the CCR hash from emp-tool (H(x) = π(σ(x)) XOR σ(x),
// Theorem 4.3 of the paper). The two children of the root are
// (k, Δ XOR k); from there the leveled correlation `Δ = XOR of all
// nodes on level i` holds for every i in [1, d] (Claim 4.1).
//
// Wire usage (Figure 4 spCOT, F_COT-hybrid model): the sender
// transmits a single κ-bit correction c_i = K_{r_i} XOR K^0_i per
// level (vs the plain GGM's two κ-bit OT messages), where K^0_i is
// the XOR of all left-side nodes on level i. The receiver uses its
// base-COT M_{r_i} to derive K^{ᾱ_i}_i = M_{r_i} XOR c_i and feed
// PuncFullEval. No OTPre layer needed.
//
// Layout matches pprf:: so the swap in spcot_{sender,recver}.h is
// a one-line change.
//
// alpha bit convention: alpha_1 is MSB (i.e. alpha_j = bit (d-j) of
// alpha) — same convention pprf::eval_receiver uses.
...
// K^{ᾱ_{i+1}}_{i+1} for i ∈ [0, d). After return, leaves[x] holds
// the correct cGGM leaf for every x != alpha; leaves[alpha] is
// zero_block (the SPCOT-layer apply_punctured_correction step
// fills it with secret_sum_f2 ⊕ XOR(known leaves), which equals
// sender's leaf at alpha XOR Δ — the COT relation).
inline void eval_receiver(int d, int alpha,
    ...
    for (int i = 2; i <= d; ++i) {
        const int parents = 1 << (i - 1);
        // Expand in place from highest parent down so on-path parent
        // (zero) writes (H(0), 0) to its children — both wrong, but
        // those slots get fixed below: one becomes K_recv-derived,
        // the other stays the new puncture and will be set next iter.
        for (int j = parents - 1; j >= 0; --j) {
            block parent = leaves[j];
            block left   = ccrh.H(parent);
            block right  = parent ^ left;
        ...
        // Zero the on-path node's two children (their values were
        // junk from the H(0) expansion above; the sibling slot will
        // be overwritten via K_recv, and the on-path slot is the
        // new puncture).
        leaves[2 * path]     = zero_block;
        leaves[2 * path + 1] = zero_block;

        // Sibling = K_recv[i-1] XOR (XOR of all alpha_bar_i-side
        // nodes on level i other than the sibling). Equivalent to
        // the formula in Figure 3.
        block sum = zero_block;
        for (int j = alpha_bar_i; j < (1 << i); j += 2)
            sum = sum ^ leaves[j];
        leaves[sibling_lvl] = sum ^ K_recv[i - 1];
        // leaves[on_path_lvl] stays zero_block (the new puncture).
