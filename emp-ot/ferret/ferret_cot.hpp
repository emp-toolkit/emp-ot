...
template<typename T>
void FerretCOT<T>::rcot(block *data, size_t num) {
	if(ot_data == nullptr) {
		ot_data = new block[n];
	...
	block *pt = data;
	size_t gened = silent_ot_left();
	if(gened > 0) {
		memcpy(pt, ot_data+ot_used, gened*sizeof(block));
		pt += gened;
	}
	size_t round_inplace = (num-gened-M) / ot_limit;
	size_t last_round_ot = num-gened-round_inplace*ot_limit;
	bool round_memcpy = last_round_ot>ot_limit?true:false;
	if(round_memcpy) last_round_ot -= ot_limit;
	for(size_t i = 0; i < round_inplace; ++i) {
		extend_f2k(pt);
		ot_used = ot_limit;
		pt += ot_limit;
	}
...
template<typename T>
size_t FerretCOT<T>::silent_ot_left() {
...
void FerretCOT<T>::write_pre_data128_to_file(void* loc, __uint128_t delta, std::string filename) {
	...
	fio.send_data(&party, sizeof(size_t));
	if(party == ALICE) fio.send_data(&delta, 16);
	fio.send_data(&n, sizeof(size_t));
	fio.send_data(&t, sizeof(size_t));
	fio.send_data(&k, sizeof(size_t));
...
__uint128_t FerretCOT<T>::read_pre_data128_from_file(void* pre_loc, std::string filename) {
	...
	int in_party;
	fio.recv_data(&in_party, sizeof(size_t));
	if(in_party != party) error("wrong party");
	__uint128_t delta = 0;
	if(party == ALICE) fio.recv_data(&delta, 16);
	size_t nin, tin, kin;
	fio.recv_data(&nin, sizeof(size_t));
	fio.recv_data(&tin, sizeof(size_t));
	fio.recv_data(&kin, sizeof(size_t));
...
template<typename T>
size_t FerretCOT<T>::byte_memory_need_inplace(size_t ot_need) {
	size_t round = (ot_need - 1) / ot_limit;
...
template<typename T>
size_t FerretCOT<T>::rcot_inplace(block *ot_buffer, size_t byte_space) {
	...
	size_t ot_output_n = byte_space - M;
	size_t round = ot_output_n / ot_limit;
	block *pt = ot_buffer;
...
template<typename T>
void FerretCOT<T>::online_sender(block *data, size_t length) {
...
template<typename T>
void FerretCOT<T>::online_recver(block *data, const bool *b, size_t length) {
	bool *bo = new bool[length];
	for(size_t i = 0; i < length; ++i) {
...
template<typename T>
void FerretCOT<T>::send_cot(block * data, size_t length) {
	rcot(data, length);
	online_sender(data, length);
}

template<typename T>
void FerretCOT<T>::recv_cot(block* data, const bool * b, size_t length) {
	rcot(data, length);
	online_recver(data, b, length);
}

template<typename T>
void FerretCOT<T>::assemble_state(void * data, size_t size) {
	unsigned char * array = (unsigned char * )data;
	memcpy(array, &party, sizeof(size_t));
	memcpy(array + sizeof(size_t), &n, sizeof(size_t));
	memcpy(array + sizeof(size_t) * 2, &t, sizeof(size_t));
	memcpy(array + sizeof(size_t) * 3, &k, sizeof(size_t));
	memcpy(array + sizeof(size_t) * 4, &Delta, sizeof(block));	
	memcpy(array + sizeof(size_t) * 4 + sizeof(block), ot_pre_data, sizeof(block)*n_pre);
	if (ot_pre_data!= nullptr)
		delete[] ot_pre_data;
	ot_pre_data = nullptr;
}
...
template<typename T>
size_t FerretCOT<T>::disassemble_state(const void * data, size_t size) {
	const unsigned char * array = (const unsigned char * )data;
	size_t n2 = 0, t2 = 0, k2 = 0, party2 = 0;
	ot_pre_data = new block[n_pre];
	memcpy(&party2, array, sizeof(size_t));
	memcpy(&n2, array + sizeof(size_t), sizeof(size_t));
	memcpy(&t2, array + sizeof(size_t) * 2, sizeof(size_t));
	memcpy(&k2, array + sizeof(size_t) * 3, sizeof(size_t));
	if(party2 != party or n2 != n or t2 != t or k2 != k) {
		return -1;
	}
	memcpy(&Delta, array + sizeof(size_t) * 4, sizeof(block));	
	memcpy(ot_pre_data, array + sizeof(size_t) * 4 + sizeof(block), sizeof(block)*n_pre);
...
template<typename T>
size_t FerretCOT<T>::state_size() {
	return sizeof(size_t) * 4 + sizeof(block) + sizeof(block)*n_pre;
}
