...
const static size_t ot_bsize = 8;
template<typename T>
class COT : public OT<T>{ public:
	...
	block Delta;
	PRG prg;
	virtual void send_cot(block* data0, size_t length) = 0;
	virtual void recv_cot(block* data, const bool* b, size_t length) = 0;
	void send(const block* data0, const block* data1, size_t length) override {
		...
		for(size_t i = 0; i < length; i+=ot_bsize) {
			for(size_t j = i; j < min(i+ot_bsize, length); ++j) {
				pad[2*(j-i)] = data[j];
	...
	void recv(block* data, const bool* r, size_t length) override {
	...
	void send_rot(block* data0, block* data1, size_t length) {
	...
	void recv_rot(block* data, const bool* r, size_t length) {
		recv_cot(data, r, length);
		block s;
		io->recv_block(&s,1);
		mitccrh.setS(s);
