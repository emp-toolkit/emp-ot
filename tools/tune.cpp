...
// shape and emits the per-build-directory tuning_local.h (path supplied by
// the tune targets) overriding exactly the knobs whose best candidate beats
// the shipped default by more than the measured noise band. The registry
// contains only LOCAL knobs (output- and wire-invariant; enforced by
// test_tuning_invariance) — AGREEMENT and SECURITY parameters have no
// override channel at all (see tuning.h).
//
...
//   emp-ot-tune --emit <path>         # also write <path> (`tune` does this)
//   emp-ot-tune --emit-if-untuned <path>
//       # auto-tune hook: sweep only if <path> is absent, empty, or still
//       # the canonical configure-time stub; manual/generated overrides stay.
...
int main(int argc, char** argv) {
	string emit_path;
	bool only_if_untuned = false;
	for (int i = 1; i < argc; ++i) {
		if (string(argv[i]) == "--emit" && i + 1 < argc) emit_path = argv[i + 1];
		if (string(argv[i]) == "--emit-if-untuned" && i + 1 < argc) {
			emit_path = argv[i + 1];
			only_if_untuned = true;
		}
	}
	// Auto-tune mode (`--emit-if-untuned`, used by the Release build hook):
	// an existing TUNED result wins — tuning is once per build directory;
	// re-tune is the explicit `tune` target or `tune-clean` + rebuild. The
	// canonical configure-time stub (or a missing/legacy-empty file) does not
	// count as tuned. Every other nonempty file is an override, including a
	// hand-written one: auto-tuning must never destroy an explicit choice.
	if (only_if_untuned && !emit_path.empty()) {
		ifstream in(emit_path);
		string first, recorded_host, line;
		const bool has_first = in && static_cast<bool>(getline(in, first));
		bool has_extra_content = false;
		while (getline(in, line)) {
			if (!line.empty()) has_extra_content = true;
			if (line.rfind("// host: ", 0) == 0)
				recorded_host = line.substr(9);
		}
		constexpr const char *kUntunedStub = "// EMP_OT_UNTUNED_STUB_V1";
		const bool is_stub = has_first && first == kUntunedStub &&
		                     !has_extra_content;
		if (has_first && !is_stub) {
			const bool generated =
			    first.rfind("// GENERATED by emp-ot-tune", 0) == 0;
			printf("emp-ot tune: %s holds %s overrides — keeping them (run "
			       "the `tune` target or `tune-clean` to re-sweep)\n",
			       emit_path.c_str(), generated ? "generated" : "manual");
			// Advisory only: a build directory normally stays on one machine,
			// but a copied or network-mounted one silently applying another
			// host's tiling is worth a loud line.
			if (generated && !recorded_host.empty() &&
			    recorded_host != cpu_brand())
				printf("WARNING: kept result was tuned on \"%s\" but this host "
				       "is \"%s\" — run the `tune` target to re-sweep\n",
				       recorded_host.c_str(), cpu_brand().c_str());
			return 0;
		}
	}
	...
	{
		// Swept for the record but NEVER auto-emitted: the tile sizes COT's
		// MITCCRH member, so an override changes the layout of every COT
		// descendant — an ABI break between differently-configured TUs. It
		// is pinned in tuning.h; no measured host has beaten the default
		// beyond noise. Re-open the override channel only together with an
		// ABI-neutral dispatch design.
		vector<Candidate> c = {cot_tile_candidate<8>(true),
		                       cot_tile_candidate<4>(false),
		                       cot_tile_candidate<16>(false)};
		Verdict v = sweep("cot_tile (ns/OT, H=2)", c);
		if (!v.define.empty()) {
			v.note += "  [informational only -- cot tile is pinned in tuning.h"
			          " (sizes COT's MITCCRH member; ABI)]";
			v.define.clear();
		}
		verdicts.push_back(v);
	}
	...
	// Write-then-rename so a concurrent reader (another build process, an
	// install step) never sees a partially written header. No includer
	// touch is needed: the per-build-dir file exists from configure time
	// onward, so compiler depfiles already track it and this rewrite alone
	// recompiles the affected TUs.
	const string tmp_path = emit_path + ".tmp";
	{
		ofstream f(tmp_path);
		if (!f) {
		...
		f << "// GENERATED by emp-ot-tune (`make tune`) " << date
		  << " -- per-build-directory override header.\n";
		f << "// host: " << cpu_brand() << "\n";
		f << "// Overrides LOCAL-class knobs only (see tuning.h); run `make\n";
		f << "// tune-clean` to return to the shipped defaults.\n";
	...
	if (rename(tmp_path.c_str(), emit_path.c_str()) != 0) {
		fprintf(stderr, "cannot rename %s to %s\n", tmp_path.c_str(),
		        emit_path.c_str());
		return 1;
	}
	printf("\nwrote %s (%d override%s) -- rebuild applies it\n",
	       emit_path.c_str(), n_override, n_override == 1 ? "" : "s");
	return 0;
}
